"""
tideon.ai CLI - Command-line interface for the tideon.ai framework.

This module provides a comprehensive CLI for interacting with tideon.ai:
...
Usage:
    ...
    teffgen run "What is 2+2?" --model Qwen/Qwen2.5-1.5B-Instruct

    # Interactive wizard (launches when no task provided)
    teffgen run
    teffgen  # Same as above

    # teffgen-agent command (similar to smolagent)
    teffgen-agent "Plan a trip to Tokyo" --model Qwen/Qwen2.5-1.5B-Instruct --tools web_search
    teffgen-agent  # Interactive mode
    ...
    teffgen tools list
    teffgen models list
    teffgen examples run basic_agent
...
# Import tideon.ai components
try:
    from teffgen import (  # noqa: F401
        Agent,
        AgentConfig,
        ConfigLoader,
        __version__,
    ...
    print("Error: tideon.ai package not found. Please install it first.")
    sys.exit(1)
...
class CLIInterface:
    """Main CLI interface for tideon.ai."""
    ...
    def interactive_wizard(self, args):
        ...
        Returns:
            Exit code
        """
        self.print_header(f"tideon.ai v{__version__} - Interactive Setup Wizard")
        self.print()

        if self.console:
            self.console.print(Panel(
                "[bold cyan]Welcome to tideon.ai Interactive Mode![/bold cyan]\n\n"
            ...
            print("=" * 60)
            print("Welcome to tideon.ai Interactive Mode!")
    ...
    def run_agent(self, args):
        ...
        self.print_header(f"tideon.ai v{__version__} - Running Task")
        ...
        try:
            ...
            if getattr(args, 'preset', None):
                from teffgen.presets import create_agent as _create_preset_agent
    ...
    def chat_mode(self, args):
        ...
        Args:
            args: Parsed command-line arguments
        """
        self.print_header(f"tideon.ai v{__version__} - Chat Mode")
    ...
    def _history_dir() -> Path:
        ...
        d = Path.home() / ".teffgen" / "history"
        d.mkdir(parents=True, exist_ok=True)
        return d

    def _save_conversation(self, history: list[dict]):
        """Save conversation history to ~/.teffgen/history/."""
    ...
    def serve_api(self, args):
        ...
        Args:
            args: Parsed command-line arguments
        """
        self.print_header(f"tideon.ai v{__version__} - API Server")
        ...
        try:
            ...
            _rate_limit = int(os.environ.get("TEFFGEN_RATE_LIMIT", "60"))  # requests/min
            ...
            expected_key = os.environ.get("TEFFGEN_API_KEY")  # None = auth disabled
            ...
            async def lifespan(app: FastAPI):
                ...
                if expected_key:
                    cli_instance.print("API key auth enabled (set via TEFFGEN_API_KEY)")
                cli_instance.print(f"Rate limit: {_rate_limit} req/min (set via TEFFGEN_RATE_LIMIT)")
                yield
            ...
            app = FastAPI(
                title="tideon.ai API",
                description="API server for tideon.ai framework. "
                            "Set TEFFGEN_API_KEY to enable authentication. "
                            "Set TEFFGEN_RATE_LIMIT to configure rate limiting (default 60 req/min).",
                version=__version__,
                lifespan=lifespan
            )
    ...
    def _config_show(self, args):
        ...
        if args.file:
            ...
            self.print("Use: teffgen config show --file <path>")
    ...
    def _models_load(self, args):
        ...
        from teffgen.models.pool import ModelPool
    ...
    def _models_status(self, args):
        ...
        # Capability registry
        from teffgen.models.capabilities import list_registered_models
    ...
    def _examples_list(self, args):
        ...
        if self.console:
            ...
            for example in sorted(examples):
                table.add_row(example, f"teffgen examples run {example}")
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description=f"tideon.ai v{__version__} - CLI for agent framework",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  teffgen run "What is the weather in Paris?" --model Qwen/Qwen2.5-3B-Instruct
  teffgen chat --model Qwen/Qwen2.5-3B-Instruct --temperature 0.8
  teffgen serve --port 8000
  teffgen config show --file configs/models.yaml
  teffgen tools list
  teffgen examples run basic_agent
        """
    )

    parser.add_argument('--version', action='version', version=f'tideon.ai {__version__}')
    ...
    subparsers.add_parser('health', help='Check tideon.ai infrastructure health')
...
def _create_plugin_scaffold(plugin_name: str, output_dir: str = ".") -> int:
    ...
    base = Path(output_dir) / f"teffgen-plugin-{plugin_name}"
    ...
    (pkg / "__init__.py").write_text(
        f'"""tideon.ai plugin: {plugin_name}"""\n'
    )
...
class {plugin_name.title().replace("_", "")}Plugin(ToolPlugin):
    name = "{plugin_name}"
    version = "{__version__}"
    description = "A custom tideon.ai tool plugin."
    tools = [ExampleTool]
'''
    )
...
[project]
name = "teffgen-plugin-{plugin_name}"
version = "{__version__}"
description = "An tideon.ai tool plugin"
requires-python = ">=3.9"
dependencies = ["teffgen"]
...
'''
    ...
    (base / "README.md").write_text(
        f"# teffgen-plugin-{plugin_name}\\n\\n"
        f"An tideon.ai tool plugin.\\n\\n"
        "## Install\\n\\n"
        "```bash\\n"
        f"pip install -e .\\n"
        "```\\n\\n"
        "The plugin will be auto-discovered by tideon.ai via entry points.\\n"
    )
...
def _handle_workflow_command(args, cli) -> int:
    ...
    from teffgen.core.workflow import WorkflowDAG
    ...
    if wf_cmd == 'validate':
        ...
        cli.print("Usage: teffgen workflow [run|validate] <file.yaml>")
        return 0


def _handle_batch_command(args, cli) -> int:
    """Handle the 'batch' CLI subcommand."""
    from teffgen.core.batch import BatchConfig, BatchRunner
...
def _handle_eval_command(args, cli) -> int:
    """Handle 'teffgen eval' subcommand."""
    from teffgen.eval import AgentEvaluator, RegressionTracker, get_suite, list_suites
    ...
    try:
        ...
        if difficulty:
            from teffgen.eval.evaluator import Difficulty
...
def _handle_compare_command(args, cli) -> int:
    """Handle 'teffgen compare' subcommand."""
    from teffgen.eval import ModelComparison, get_suite
    from teffgen.eval.evaluator import ScoringMode
...
def _handle_resume_command(args, cli) -> int:
    """Handle 'teffgen resume' command."""
    from teffgen import Agent, AgentConfig
    from teffgen.core.checkpoint import CheckpointManager
...
def _handle_sessions_command(args, cli) -> int:
    """Handle 'teffgen sessions' subcommands."""
    from teffgen.core.session import SessionManager
    mgr = SessionManager()
    ...
    if cmd == 'cleanup':
        ...
        return 0
    cli.print("Usage: teffgen sessions [list|delete|export|cleanup]")
    return 1
...
def main():
    ...
    if getattr(args, 'completion', None):
        from teffgen.completion import get_completion
    ...
    try:
        if args.command == 'run':
            ...
            from teffgen.debug.inspector import run_debug_cli
            run_debug_cli(
                task=args.task,
...
def agent_main():
    """
    Entry point for teffgen-agent CLI (similar to smolagent).

    A generalist command to run a multi-step agent that can be equipped with various tools.

    Usage:
        # Run with direct prompt and options
        teffgen-agent "Plan a trip to Tokyo" --model Qwen/Qwen2.5-1.5B-Instruct --tools web_search calculator
...
def web_agent_main():
    """
    Entry point for web agent CLI (teffgen-web).

    A specialized agent for web browsing tasks.

    Usage:
        teffgen-web "go to example.com and get the page title"
        teffgen-web  # Interactive mode
    """
    import sys
    ...
    if len(sys.argv) > 1 and not sys.argv[1].startswith('-'):
        ...
        print(f"tideon.ai Web Agent v{__version__}")
        print()
        print("Usage:")
        print("  teffgen-web \"<task>\"           Run a web task")
        print("  teffgen-web --model <model>    Specify model")
        print()
        print("Example:")
        print("  teffgen-web \"Search for the latest Python release\"")
        print()
        return
