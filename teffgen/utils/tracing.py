"""
OpenTelemetry tracing integration for tideon.ai.
...
Configuration via environment variables:
    ...
    OTEL_SERVICE_NAME: Service name (default: "teffgen")

Usage:
    from teffgen.utils.tracing import get_tracer, setup_tracing
...
def setup_tracing(
    ...
    """
    Set up OpenTelemetry tracing for tideon.ai.
    ...
    Args:
        service_name: Service name for traces (default: env or "teffgen")
    ...
    service_name = service_name or os.environ.get("OTEL_SERVICE_NAME", "teffgen")
...
# ---------------------------------------------------------------------------
# Span helpers — create child spans with standard tideon.ai attributes
# ---------------------------------------------------------------------------
...
def trace_agent_run(agent_name: str, task: str, run_id: str | None = None):
    ...
    tracer = get_tracer()
    attrs: dict[str, Any] = {
        "teffgen.agent.name": agent_name,
        "teffgen.agent.task": task[:500],
    }
    if run_id:
        attrs["teffgen.run_id"] = run_id
...
def trace_tool_execute(tool_name: str, tool_input: str):
    ...
    return tracer.start_as_current_span(
        "tool.execute",
        attributes={
            "teffgen.tool.name": tool_name,
            "teffgen.tool.input": tool_input[:500],
        },
    )
...
def trace_model_generate(model_name: str, prompt_tokens: int = 0):
    ...
    tracer = get_tracer()
    attrs: dict[str, Any] = {"teffgen.model.name": model_name}
    if prompt_tokens:
        attrs["teffgen.model.prompt_tokens"] = prompt_tokens
