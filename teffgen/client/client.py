"""tideon.ai client — sync (requests) and async (httpx) HTTP client.

The client speaks the tideon.ai API server protocol. For OpenAI-compatible
...
class TeffgenClient:
    """Client for an tideon.ai API server.

    Parameters
    ----------
    base_url:
        Base URL of the tideon.ai API server (e.g. ``http://localhost:8000``).
    api_key:
    ...
    def _raise_for_status(self, status: int, payload: Any) -> None:
        ...
        if status in (401, 403):
            raise TeffgenAuthError(msg, status_code=status, payload=payload)
        if status == 429:
            raise TeffgenRateLimitError(msg, status_code=status, payload=payload)
        if 500 <= status < 600:
            raise TeffgenServerError(msg, status_code=status, payload=payload)
        raise TeffgenAPIError(msg, status_code=status, payload=payload)

    def _is_retryable(self, exc: BaseException) -> bool:
        if isinstance(exc, TeffgenConnectionError | TeffgenTimeoutError):
            return True
        if isinstance(exc, TeffgenServerError | TeffgenRateLimitError):
            return True
        return False
    ...
    def _request_sync(self, method: str, path: str, json_body: dict | None = None) -> Any:
        try:
            ...
            raise TeffgenClientError(  # noqa: F821
                "The 'requests' package is required for sync TeffgenClient calls"
            ) from e
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                last_exc = TeffgenConnectionError(str(e))
            except requests.exceptions.Timeout as e:
                last_exc = TeffgenTimeoutError(str(e))
            except (TeffgenServerError, TeffgenRateLimitError) as e:
                last_exc = e
            except TeffgenAPIError:
                raise  # non-retryable
    ...
    async def _request_async(
        ...
        try:
            ...
            raise TeffgenClientError(  # noqa: F821
                "The 'httpx' package is required for async TeffgenClient calls"
            ) from e
    ...
    def chat(
        ...
        **kwargs: Any,
        ...
        body: dict = {
            "model": model or "teffgen-default",
    ...
    def embed(
        ...
        if not isinstance(payload, dict) or "data" not in payload:
            raise TeffgenAPIError("Malformed embeddings response", payload=payload)
    ...
    def chat_stream_sync(self, message: str, model: str | None = None) -> Iterator[str]:
        ...
        try:
            ...
            raise TeffgenClientError("requests required for sync streaming") from e  # noqa: F821
    ...
    async def chat_stream(
        ...
        try:
            ...
            raise TeffgenClientError("httpx required for async streaming") from e  # noqa: F821
    ...
    def _parse_chat(payload: Any) -> ChatResponse:
        if not isinstance(payload, dict):
            raise TeffgenAPIError("Malformed chat response", payload=payload)
        content = ""
        tool_calls: list[Any] = []
...
from teffgen.client.exceptions import TeffgenClientError  # noqa: E402
