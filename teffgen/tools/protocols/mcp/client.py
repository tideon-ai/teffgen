...
class MCPToolBridge(BaseTool):
    """
    Bridge that wraps an MCP tool as an tideon.ai BaseTool.
    ...
    def __init__(self, mcp_tool: MCPTool, client: "MCPClient"):
        ...
        """
        # Convert MCP tool schema to tideon.ai ParameterSpec list
        params = []
...
class MCPClient:
    ...
    - MCP tool → tideon.ai tool bridge
    ...
    # --- MCP Tool → tideon.ai Tool Bridge (3.2.2) ---

    def get_teffgen_tools(self) -> list[BaseTool]:
        """
        Get MCP tools as tideon.ai BaseTool instances.
