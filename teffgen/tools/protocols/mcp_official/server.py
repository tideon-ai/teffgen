...
This module provides a standards-compliant MCP server that exposes tideon.ai
...
@dataclass
class TeffgenMCPServerConfig:
    """Configuration for tideon.ai MCP server."""
    name: str = "teffgen-tools"
    version: str = "1.0.0"
    instructions: str = "MCP server exposing tideon.ai tools for AI agents"
    tools_registry: ToolRegistry | None = None


class TeffgenMCPServer:
    """
    Official MCP server implementation for tideon.ai.

    This server uses the official MCP Python SDK (FastMCP) to expose
    tideon.ai tools in a standards-compliant way.
    ...
    def __init__(
        ...
        """
        Initialize tideon.ai MCP server.
        ...
        self.instructions = instructions or (
            "MCP server exposing tideon.ai tools for AI agents. "
        ...
        logger.info(f"Initialized tideon.ai MCP Server: {self.name} v{self.version}")
...
def create_server(
    ...
    """
    Create an tideon.ai MCP server instance.
    ...
    Returns:
        tideon.ai MCP server instance
    """
    return TeffgenMCPServer(
        name=name,
        version=version,
