"""
tideon.ai Plugin System — Load external tool plugins via entry points and directories.

Plugin discovery sources (in order of precedence):
1. ``teffgen.plugins`` entry-point group (installed packages)
2. ``~/.teffgen/plugins/`` directory (user-local plugins)
3. ``TEFFGEN_PLUGINS_DIR`` environment variable (custom directory)
...
from teffgen.tools.registry import ToolRegistry, get_registry
...
class PluginManager:
    ...
    def discover_entry_points(self) -> list[str]:
        """Load plugins from the ``teffgen.plugins`` entry-point group."""
        loaded: list[str] = []
        try:
            from importlib.metadata import entry_points
            eps = entry_points(group="teffgen.plugins")
        except Exception:
            logger.debug("No entry points found for teffgen.plugins")
            return loaded
    ...
    def discover_user_dir(self) -> list[str]:
        """Load plugins from ``~/.teffgen/plugins/``."""
        user_dir = Path.home() / ".teffgen" / "plugins"
        return self._discover_directory(user_dir)

    def discover_env_dir(self) -> list[str]:
        """Load plugins from ``TEFFGEN_PLUGINS_DIR`` environment variable."""
        env_path = os.environ.get("TEFFGEN_PLUGINS_DIR")
        if not env_path:
            return []
    ...
    def _load_module_from_file(path: Path):
        ...
        module_name = f"teffgen_plugin_{path.stem}"
