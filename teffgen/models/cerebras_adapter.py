"""
Cerebras Cloud SDK adapter for tideon.ai.
...
from teffgen.models.cerebras_models import (
...
class CerebrasAdapter(BaseModel):
    ...
    Wraps the ``cerebras-cloud-sdk`` with the standard tideon.ai BaseModel
    interface. Supports:
    ...
    - Per-request cost tracking via :class:`~teffgen.models._cost.CostTracker`
    - Per-model rate-limit coordination

    Args:
        model_name: Cerebras model ID. Must be a key in
            :data:`~teffgen.models.cerebras_models.CEREBRAS_MODELS`.
        ...
        enable_rate_limiting: If ``True`` (default), acquire / record calls
            via the built-in :class:`~teffgen.models._rate_limit.RateLimitCoordinator`.
        enable_cost_tracking: If ``True`` (default), record token usage in
            the global :class:`~teffgen.models._cost.CostTracker`.
    ...
    def load(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Install with: pip install 'teffgen[cerebras]'"
            ) from exc
