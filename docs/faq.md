...
**Q: Do I need a GPU to run tideon.ai?**
...
**Q: What models work best with tideon.ai?**
A: tideon.ai is optimized for Small Language Models (1B-7B params). Recommended:
...
**Q: Does tideon.ai require any paid APIs?**
...
**Q: How do I use tideon.ai with OpenAI/Claude instead of local models?**
A:
```python
...
**Q: ImportError when importing tideon.ai**
A: Ensure you installed with `pip install teffgen` (or `pip install -e .` for development). Check `python -c "import teffgen; print(teffgen.__version__)"`.
