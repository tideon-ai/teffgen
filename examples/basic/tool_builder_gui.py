"""
tideon.ai Tool Builder — Visual GUI for creating custom tools.
...
def generate_tool_code(
    ...
    code = textwrap.dedent(f'''\
        """
        {tool_desc or 'Custom tool generated by tideon.ai Tool Builder.'}
        """
        ...
        class {class_name}(BaseTool):
            ...
            Generated by tideon.ai Tool Builder GUI.
            """
...
def generate_plugin_code(
    ...
    plugin_code = textwrap.dedent(f'''\
        """
        tideon.ai Plugin: {tool_name or 'My Custom Tool'}

        Install:
            1. Save this file to ~/.teffgen/plugins/{plugin_name}.py
            2. Or register via entry points in pyproject.toml

        Usage:
            from teffgen.tools.plugin import PluginManager
            mgr = PluginManager()
            mgr.discover_all()  # auto-discovers from ~/.teffgen/plugins/
        """
...
def build_app():
    ...
    with gr.Blocks(title="tideon.ai Tool Builder") as app:

        gr.Markdown(
            "# tideon.ai Tool Builder\n"
            "Visually create, test, and export custom tools for the tideon.ai framework."
        )
...
""")
            ...
            with gr.Tab("5. Export"):
                gr.Markdown(
                    ...
                    "or as a **Plugin** ready to drop into `~/.teffgen/plugins/`."
                )
...
1. Save the generated tool class to `teffgen/tools/builtin/my_tool.py`
2. Import it in your agent code:
   ```python
   from teffgen.tools.builtin.my_tool import MyCustomTool
   agent = Agent(config=AgentConfig(tools=[MyCustomTool()], ...))
   ```

**Option B: As a plugin**
1. Save the generated plugin to `~/.teffgen/plugins/my_tool.py`
2. The plugin system auto-discovers it:
   ```python
   from teffgen.tools.plugin import discover_plugins
   discover_plugins()  # auto-loads from ~/.teffgen/plugins/
   ```
...
def main():
    parser = argparse.ArgumentParser(description="tideon.ai Tool Builder GUI")
